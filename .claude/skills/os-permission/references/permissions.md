...
`project.yml` 是 iOS / macOS 權限宣告的唯一設定來源；`apps/*/Info.plist` 與 `*.entitlements` 均由 XcodeGen 產生。Windows 端以 `apps/windows/electron-builder.yml` + `apps/windows/package.json` + AppX manifest 為來源。下表同時涵蓋使用者授權、必要宣告、macOS sandbox entitlement、Windows capability / Firewall 規則，以及專案依賴但不會出現獨立 prompt 的系統能力。
...
| Name | Description | What is it for in current project? |
| --- | --- | --- |
| `Windows Firewall Inbound Rule` | `netsh advfirewall firewall add rule` 於 NSIS installer 階段註冊：UDP 5353（Bonjour / mDNS multicast）與 TCP `_iphonesync._tcp` / `_iphonesync-pair._tcp` sync port。解除安裝時同步移除。 | 讓 iPhone 在同一 LAN 透過 Bonjour 找到 Windows receiver；沒有這條規則會被 Windows Defender Firewall 預設阻擋。 |
| `Windows AppX Capability — internetClient` | AppX manifest `<Capability Name="internetClient" />` 或 AppX unpackaged fallback；outbound TCP/TLS。 | 讓 receiver 主動發起 outbound TLS-PSK 連線（如需）。 |
| `Windows AppX Capability — privateNetworkClientServer` | AppX manifest `<Capability Name="privateNetworkClientServer" />`；同時允許 inbound 與 outbound private network。 | 讓 `tls.createServer` 監聽 sync port，並讓 Bonjour / mDNS 在 private network 通訊。目前專案**未啟用 AppContainer**，此 capability 預留給未來 Microsoft Store 上架。 |
| `Windows Loopback Exemption (CheckNetIsolation)` | AppContainer 化後若需要 multicast，必須用 `CheckNetIsolation.exe` 給 loopback exemption。 | 預留；目前未啟用 AppContainer，不需要。 |
| `Windows Defender SmartScreen Reputation` | 第一次下載的 `.exe` 會跳出 SmartScreen 警告；EV certificate 直接通過，OV 需要累積 reputation 或送 Microsoft 提交表單。 | 影響首次安裝體驗；Authenticode 簽章是緩解手段，不是 prompt 等級的權限。 |
| `Windows Auto-Start via HKCU\Run` | Electron `app.setLoginItemSettings({ openAtLogin: true })` 寫入 `HKCU\Software\Microsoft\Windows\CurrentVersion\Run`。 | 讓 Windows receiver 開機自動啟動；解除安裝時需 NSIS custom script 清乾淨。 |
| `Windows DPAPI (Data Protection API)` | `safeStorage` 或自訂實作以 `CryptProtectData` / `CryptUnprotectData` 加密敏感資料；不需要獨立 prompt。 | 保存 pairing 後的 256-bit PSK 與 opaque peer identity，不將 secret 寫入明文檔案。相當於 macOS / iOS 端 Keychain 的角色。 |
| `Windows UAC (User Account Control)` | 安裝 NSIS installer 第一次執行會跳 UAC 對話框（perMachine: false 時不一定需要）。 | NSIS 目前設 `perMachine: false`，大多數情況以使用者權限安裝即可。 |

## 共同（跨平台）

| Name | Description | What is it for in current project? |
| --- | --- | --- |
| `App-Private Secret Store` | iOS / macOS 使用標準 Keychain Services；Windows 使用 DPAPI 或 `safeStorage`。不會出現獨立 privacy prompt。目前沒有跨 target 分享，因此不需要 `keychain-access-groups` entitlement 或 Windows credential manager sharing。 | 各 App 分別保存 pairing 後的 256-bit PSK 與 opaque peer identity，不將 secret 寫入 `UserDefaults`、SwiftData 或明文檔案。 |
| `Bonjour / mDNS Multicast UDP 5353` | Apple 端走 `NWBrowser` / `NWListener`；Windows 端走 `multicast-dns`。iOS / macOS 透過 `NSBonjourServices` 宣告；Windows 由 installer 註冊 Firewall 規則。 | 三端互相發現；不在 `NSBonjourServices` 內的 service type 會被 OS 拒絕。 |
...
- 不啟用 AppContainer（Windows 對應 macOS App Sandbox 的機制）；若有，會需要再加 `runFullTrust` / capability 評估。
- 不使用 Windows Credential Manager 共用 secret；secret 走 DPAPI / safeStorage。
- 不使用 `location`、相機、麥克風、地理位置、人類介面裝置、藍牙、NFC。
- 不上架 Microsoft Store 就免 AppX manifest；目前的 NSIS + portable 散佈路徑不需要它。
...
- [AppX package manifest capabilities](https://learn.microsoft.com/en-us/uwp/schemas/appxpackage/uapmanifestschema/element-capability)
...
- [Authenticode signing](https://learn.microsoft.com/en-us/windows/security/application-security/application-control/operation-signing)
- [SmartScreen reputation](https://learn.microsoft.com/en-us/windows/security/operating-system-security/virus-and-threat-protection/microsoft-defender-smartscreen/)
- [Electron setLoginItemSettings](https://www.electronjs.org/docs/latest/api/app#appsetloginitemsettingssettings-windows)
- [Electron safeStorage](https://www.electronjs.org/docs/latest/api/safe-storage)
- [DPAPI overview](https://learn.microsoft.com/en-us/windows/win32/api/dpapi/)